...
import { AppRouter, appRouter } from ':aws-nx-poc/api';
...
> extends RestApi<Operations, TIntegrations> {
  ...
  constructor(scope: Construct, id: string, props: ApiProps<TIntegrations>) {
    const authorizerFunction = new Function(scope, 'ApiAuthorizerFunction', {
      runtime: Runtime.NODEJS_LATEST,
      handler: 'index.handler',
      code: Code.fromAsset(
        url.fileURLToPath(
          new URL(
            '../../../../../../dist/packages/api/bundle/authorizer',
            import.meta.url,
          ),
        ),
      ),
    ...
    super(scope, id, {
      ...
      defaultMethodOptions: {
        ...
        authorizer: new TokenAuthorizer(scope, 'ApiTokenAuthorizer', {
