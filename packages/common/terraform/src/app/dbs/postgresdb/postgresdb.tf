...
variable "docker_image_tag" {
  ...
  type        = string
  default     = "ts-rdb-terraform-postgresdb-migration:latest"
}
...
locals {
  create_db_user_bundle_path = "${path.module}/../../../../../../../dist/packages/postgresdb/bundle/create-db-user"
  migration_function_name = "postgresdb-migration-${random_string.suffix.result}"
  create_db_user_function_name = "postgresdb-create-db-user-${random_string.suffix.result}"
...
data "archive_file" "create_db_user_zip" {
  ...
  output_path = "${path.module}/../../../../../../../dist/packages/common/terraform/dbs/postgresdb/create-db-user.zip"
}

resource "aws_s3_object" "create_db_user_zip" {
  bucket      = var.asset_bucket_name
  key         = "dbs/postgresdb/${data.archive_file.create_db_user_zip.output_base64sha256}.zip"
...
resource "aws_ecr_repository" "migration_handler" {
  ...
  name                 = "postgresdb-migration-${random_string.suffix.result}"
...
resource "aws_iam_role" "migration_handler" {
  name = "PostgresdbMigrationHandlerRole-${random_string.suffix.result}"
...
resource "aws_iam_role" "create_db_user" {
  name = "PostgresdbCreateDbUserRole-${random_string.suffix.result}"
...
resource "aws_security_group" "migration_handler" {
  name_prefix = "postgresdb-migration-"
...
resource "aws_security_group" "create_db_user" {
  name_prefix = "postgresdb-create-db-user-"
