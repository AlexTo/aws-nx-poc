...
variable "docker_image_tag" {
  ...
  type        = string
  default     = "ts-rdb-terraform-mysqldb-migration:latest"
}
...
locals {
  create_db_user_bundle_path = "${path.module}/../../../../../../../dist/packages/mysqldb/bundle/create-db-user"
  migration_function_name = "mysqldb-migration-${random_string.suffix.result}"
  create_db_user_function_name = "mysqldb-create-db-user-${random_string.suffix.result}"
...
data "archive_file" "create_db_user_zip" {
  ...
  output_path = "${path.module}/../../../../../../../dist/packages/common/terraform/dbs/mysqldb/create-db-user.zip"
}

resource "aws_s3_object" "create_db_user_zip" {
  bucket      = var.asset_bucket_name
  key         = "dbs/mysqldb/${data.archive_file.create_db_user_zip.output_base64sha256}.zip"
...
resource "aws_ecr_repository" "migration_handler" {
  ...
  name                 = "mysqldb-migration-${random_string.suffix.result}"
...
resource "aws_iam_role" "migration_handler" {
  name = "MysqldbMigrationHandlerRole-${random_string.suffix.result}"
...
resource "aws_iam_role" "create_db_user" {
  name = "MysqldbCreateDbUserRole-${random_string.suffix.result}"
...
resource "aws_security_group" "create_db_user" {
  name_prefix = "mysqldb-create-db-user-"
