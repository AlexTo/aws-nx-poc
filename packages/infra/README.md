...
Run `pnpm nx build @aws-nx-poc/infra [--skip-nx-cache]` to build the application.

## Running unit tests

Run `pnpm nx test @aws-nx-poc/infra` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @aws-nx-poc/infra --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @aws-nx-poc/infra`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @aws-nx-poc/infra --configuration=fix`

## Deploy to AWS

### Deploy all Stacks

Run `pnpm nx deploy @aws-nx-poc/infra --all`

### Deploy a single Stack

Run `pnpm nx deploy @aws-nx-poc/infra [stackName]`
...
Run `pnpm nx deploy @aws-nx-poc/infra --hotswap --all`
...
```typescript
import { suppressRules } from ':aws-nx-poc/common-constructs';
