...
Run `pnpm nx build @aws-nx-poc/game-ui [--skip-nx-cache]` to build the application.

## Run dev server

Run `pnpm nx serve @aws-nx-poc/game-ui`

## Running unit tests

Run `pnpm nx test @aws-nx-poc/game-ui` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @aws-nx-poc/game-ui --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @aws-nx-poc/game-ui`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @aws-nx-poc/game-ui --configuration=fix`
...
`pnpm nx run @aws-nx-poc/game-ui:load:runtime-config`
