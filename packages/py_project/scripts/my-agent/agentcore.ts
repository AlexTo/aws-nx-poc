// Resolves the deployed MyAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  if (!arn) {
    throw new Error(
      `No deployed agent named 'MyAgent' found in runtime configuration (application ${application}).`,
    );
  }
